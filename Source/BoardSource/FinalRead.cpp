...
FinalRead::FinalRead(Board board) :Read(board)
{
...
void FinalRead::readRev()
{
	...
	for (int i = 1; i <= board.getAbleNumber(); i++)
	{
		...
		{
		case 0:
			pointSheet->writeScore(i, alfaBetaRev(-10000, 10000));
			break;
		case 4:
			board.pass();
			pointSheet->writeScore(i, alfaBetaRev(-10000, 10000));
			...
			break;
		default:
			pointSheet->writeScore(i, board.getNumber(2) - board.getNumber(1));
			count++;
			break;
		}
		board.undo();
...
}int FinalRead::alfaBetaRev(int alfa, int beta)
{
	if (board.getTurnPlayer() == 1)
	{
		for (int i = 1; i <= board.getAbleNumber(); i++)
		{
			...
			{
			case 0:
				alfa = fmax(alfa, alfaBetaRev(alfa, beta));
				break;
			case 4:
				board.pass();
				alfa = fmax(alfa, alfaBetaRev(alfa, beta));
				...
				break;
			default:
				alfa = board.getNumber(2) - board.getNumber(1);
				count++;
				break;
			}
			board.undo();
	...
	if (board.getTurnPlayer() == 2)
	{
		for (int i = 1; i <= board.getAbleNumber(); i++)
		{
			...
			{
			case 0:
				beta = fmin(beta, alfaBetaRev(alfa, beta));
				break;
			case 4:
				board.pass();
				beta = fmin(beta, alfaBetaRev(alfa, beta));
				...
				break;
			default:
				beta = board.getNumber(2) - board.getNumber(1);
				count++;
				break;
			}
			board.undo();
