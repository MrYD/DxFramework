...
struct MouseButton
{
	bool input;				   	   //ボタンが押されている間
	bool up;					   //ボタンが押された瞬間
	bool dawn;					   //ボタンが離された瞬間
	Vector2 lastDown;			   //最後に押された座標
	Vector2 lastUp;				   //最後に離された座標
};

struct Mouse
{
	Vector2 position;				//マウス座標
	Vector2 speed;                  //マウス座標の変化量
	int wheelRotVol;                //マウス回転量
	int buttonInput;                //何れかのボタンが押されている間
	int buttonUp;                   //何れかのボタンが押された瞬間
	int buttonDawn;                 //何れかのボタンが離された瞬間
	MouseButton left;
	MouseButton right;
	MouseButton middle;
};
